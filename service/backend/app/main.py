"""FastAPI application for LEDiff HDR web service."""
...
LEDIFF_WEIGHTS_DIR = os.environ.get("LEDIFF_WEIGHTS_DIR", "/app/weights")
...
def _discover_models(weights_dir: str) -> dict:
    """Discover available models in the weights directory.

    Expected layout:
        weights/highlight/sd_model/  + weights/highlight/merge_model.pth
        weights/shadow/sd_model/     + weights/shadow/merge_model.pth
    """
    available = {}
    for model_type in ("highlight", "shadow"):
        model_dir = os.path.join(weights_dir, model_type, "sd_model")
        fusion_path = os.path.join(weights_dir, model_type, "merge_model.pth")
        if os.path.isdir(model_dir) and os.path.isfile(fusion_path):
            available[model_type] = {
                "model_dir": model_dir,
                "fusion_weights": fusion_path,
            }
    return available
...
async def lifespan(app: FastAPI):
    ...
    available_models = _discover_models(LEDIFF_WEIGHTS_DIR)
    if not available_models:
        msg = (
            "\n" + "=" * 60 + "\n"
            "  No LEDiff models found!\n\n"
            "  Expected layout in " + LEDIFF_WEIGHTS_DIR + ":\n"
            "    highlight/\n"
            "      sd_model/           # Stable Diffusion checkpoint\n"
            "      merge_model.pth     # FeatureFusion weights\n"
            "    shadow/\n"
            "      sd_model/\n"
            "      merge_model.pth\n\n"
            "  Run:  ./download_weights.sh\n"
            "  Or see README.md for manual installation.\n"
            + "=" * 60
        )
        logger.error(msg)
        raise FileNotFoundError("No LEDiff models found. See logs for details.")

    logger.info("Found models: %s", list(available_models.keys()))

    from .inference import LEDiffPipeline
    app.state.pipeline = LEDiffPipeline(LEDIFF_WEIGHTS_DIR, available_models)
    ...
    logger.info("LEDiff service ready on port 8001. Available models: %s", list(available_models.keys()))
    yield
...
app = FastAPI(title="LEDiff Service", lifespan=lifespan)
...
async def upload(file: UploadFile = File(...)):
    ...
    # Handle .npy files for generation mode
    if ext == "npy":
        import io
        try:
            arr = np.load(io.BytesIO(img_bytes))
        except Exception:
            raise HTTPException(400, "Invalid .npy file")
        ...
        return UploadResponse(
            ...
            npy_shape=list(arr.shape),
        )

    # Standard image upload (ITM mode)
    try:
...
async def generate(job_id: str, req: GenerateRequest):
    ...
    if job.input_bytes is None:
        raise HTTPException(404, "Input data not found")

    if job.stage not in ("pending", "complete", "error", "cancelled"):
        raise HTTPException(409, "Job is already queued or processing")

    # Validate mode vs input type
    ext = job.filename.rsplit(".", 1)[-1].lower() if "." in job.filename else ""
    if req.mode == "generation" and ext != "npy":
        raise HTTPException(400, "Generation mode requires .npy input")
    if req.mode == "itm" and ext == "npy":
        raise HTTPException(400, "ITM mode requires an image (JPEG/PNG), not .npy")

    # Validate requested model is available
    if req.model_type not in app.state.pipeline.available_model_types:
        raise HTTPException(
            400,
            f"Model '{req.model_type}' not available. "
            f"Available: {app.state.pipeline.available_model_types}",
        )
...
async def download(job_id: str):
    ...
    from .inference import save_exr
